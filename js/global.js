...
// --- I18N MANAGER (Global, V3: trailing-slash safe + section prefix fallback + edge-middleware cookie sync) ---
(function() {
    // NOTE: cookie name 'pirabel_lang' MUST match middleware.js for edge geolocation to respect user choice.
    const LANG_COOKIE = 'pirabel_lang';
    const LANG_LS_KEY = 'pirabel_pref_lang'; // legacy localStorage key kept for backward compat
    ...
    const URL_MAP = {
        ...
        '/avis':             '/en/avis',
        '/carrieres':        '/en/carrieres',
        '/mentions-legales': '/en/mentions-legales',
        '/politique-confidentialite': '/en/politique-confidentialite',
        '/blog':             '/en/blog',
        '/guides':           '/en/guides',
        '/rendez-vous':      '/en/rendez-vous',
        '/faq':              '/en/faq',
        '/':                 '/en/'
    };

    // Section prefixes — for sub-pages where there's no exact 1:1 match, fall back to the EN agency root.
    ...
    function setLangPreference(lang) {
        ...
        // Set cookie matching middleware (pirabel_lang)
        document.cookie = LANG_COOKIE + '=' + lang + '; path=/; expires=' + expires + '; SameSite=Lax';
    }

    // Backfill cookie from legacy localStorage key on first page load.
    try {
        const stored = localStorage.getItem(LANG_LS_KEY);
        if (stored && document.cookie.indexOf(LANG_COOKIE + '=') === -1) {
    ...
    window.switchLanguage = function(lang) {
        ...
        if (lang === 'en') {
            // Already EN — just refresh to apply preference cookie
            ...
            // Already FR — just refresh to apply preference cookie (so middleware no longer redirects)
