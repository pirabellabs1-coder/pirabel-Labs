...
for (const f of files) {
  ...
  if (h.includes('/img/favicon.png?v=elan')) { h = h.replace(/\/img\/favicon\.png/g, '/img/logo.png?v=elan2'); fav++; }
  // 2) préchargement prioritaire du logo (LCP) — une seule fois, juste avant </head>
  if (!h.includes('rel="preload" as="image" href="/img/logo.png?v=elan2"') && h.includes('</head>')) {
    h = h.replace('</head>', '<link rel="preload" as="image" href="/img/logo.png?v=elan2" fetchpriority="high">\n</head>');
    pre++;
  }
