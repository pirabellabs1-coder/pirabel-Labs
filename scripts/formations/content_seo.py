    ...
    # Modules 3, 4, 5 - structure complete mais lessons en placeholder pour cette session
    {
        ...
        'lessons': [
            {'title': "Title et meta description : votre vitrine sur Google", 'duration': 20,
             'content_html': "<p><em>Cette leçon détaillée sera enrichie prochainement (contenu prévu : ~2500 mots couvrant patron des titles, longueur optimale, mots-clés positions, A/B testing CTR, descriptions meta persuasives, balises Open Graph et Twitter Card, exemples concrets pour 5 industries).</em></p><p>En attendant, retenez les essentiels :</p><ul><li>Title : 50-60 caractères, mot-clé principal en début, marque à la fin</li><li>Description : 140-160 caractères, contient un CTA, complete (et n'imite pas) le title</li><li>1 title unique par page (jamais de duplication)</li><li>OG title + Twitter title peuvent différer du title HTML pour optimiser le partage social</li></ul>"},
            {'title': "Structure H1/H2/H3 et hierarchie semantique", 'duration': 18,
             'content_html': "<p><em>Leçon détaillée à venir : ~2500 mots sur les bonnes pratiques de hiérarchie, l'impact SEO, l'accessibilité, les erreurs courantes (multiple H1, H4 sans H3 parent), comment structurer un article long, l'impact sur les featured snippets.</em></p><p>Règles clés :</p><ul><li>1 seul H1 par page, contenant le mot-clé principal</li><li>H2 pour les sections majeures, H3 pour les sous-sections</li><li>Jamais sauter de niveau (pas de H4 directement après un H2)</li><li>Chaque H2/H3 doit pouvoir être lu indépendamment et donner du sens</li></ul>"},
            {'title': "Optimisation du contenu (longueur, NLP, EEAT, intent)", 'duration': 22,
             'content_html': "<p><em>Leçon enrichie à venir : ~3000 mots couvrant la longueur optimale par type de contenu, les principes NLP que Google applique, le framework EEAT (Experience, Expertise, Authoritativeness, Trustworthiness), comment satisfaire l'intent, ajouter des éléments différenciants (data originale, exemples, screenshots).</em></p><p>Points essentiels :</p><ul><li>Longueur optimale = moyenne des top 10 actuels +20%</li><li>EEAT : citer ses sources, biographie auteur, expertise prouvée</li><li>Originalité : data unique, opinion forte, perspective non couverte</li><li>Multimedia : images, vidéos, infographies augmentent le dwell time</li></ul>"},
            ...
            {'title': "Maillage interne strategique : la structure en piliers", 'duration': 15,
             'content_html': "<p><em>Leçon enrichie à venir : ~2500 mots détaillant la structure pillar-cluster, l'anchor text optimal, la profondeur de clic, la distribution du PageRank interne, comment auditer son maillage existant, outils de visualisation (Screaming Frog, Sitebulb).</em></p><p>Principes clés :</p><ul><li>Chaque page accessible en max 3 clics depuis l'accueil</li><li>Anchor text descriptif (jamais 'cliquez ici')</li><li>Pages piliers reçoivent le plus de liens internes</li><li>Cluster pages linkent vers la page pilier de leur thématique</li><li>Audit régulier : Screaming Frog pour identifier les pages orphelines</li></ul>"},
        ],
    },
    {
        ...
        'lessons': [
            {'title': "Sitemap XML et robots.txt : guider Google", 'duration': 16,
             'content_html': "<p><em>Leçon enrichie à venir : ~2200 mots détaillés sur la structure d'un sitemap.xml propre, sitemap index pour gros sites, soumission Search Console, hreflang dans le sitemap, robots.txt syntax, erreurs fréquentes (bloquer accidentellement /css/ et /js/), priorité et lastmod.</em></p><ul><li>sitemap.xml liste TOUTES les URLs indexables</li><li>robots.txt = directives pour les bots (autoriser/bloquer)</li><li>Soumettre les 2 dans Google Search Console</li><li>Mettre à jour sitemap automatiquement à chaque publication</li></ul>"},
            {'title': "HTTPS, vitesse, Core Web Vitals : facteurs critiques", 'duration': 22,
             'content_html': "<p><em>Leçon enrichie à venir : ~3000 mots sur HTTPS (Let's Encrypt, certificats), métriques Core Web Vitals (LCP, FID/INP, CLS), outils de mesure (PageSpeed Insights, Lighthouse, Chrome UX Report), optimisations concrètes (CDN, cache, fonts, images, JS bundle splitting).</em></p><ul><li>HTTPS obligatoire (Chrome marque les sites HTTP comme non sécurisés)</li><li>LCP cible : &lt; 2.5s (Largest Contentful Paint)</li><li>INP cible : &lt; 200ms (Interaction to Next Paint, remplace FID en 2024)</li><li>CLS cible : &lt; 0.1 (Cumulative Layout Shift)</li></ul>"},
            {'title': "Mobile-first indexing : votre site doit etre mobile-perfect", 'duration': 15,
             'content_html': "<p><em>Leçon enrichie à venir : ~2200 mots sur le mobile-first index (depuis 2019), responsive design, viewport meta, touch targets 48px minimum, fonts lisibles 16px+, test mobile-friendly Google, performance mobile spécifique (3G simulation).</em></p><ul><li>Google indexe la version mobile en priorité (depuis 2019)</li><li>Si votre version mobile est inférieure à la desktop, vous perdez en ranking</li><li>Test : https://search.google.com/test/mobile-friendly</li><li>Performance critique sur 3G/4G dans les pays émergents (Bénin)</li></ul>"},
            {'title': "Erreurs 404, redirects 301, canonical : eviter les pieges", 'duration': 14,
             'content_html': "<p><em>Leçon enrichie à venir : ~2000 mots sur la détection des 404 (Search Console, Screaming Frog), redirects 301 pour le contenu déplacé, canonical pour gérer les duplications légitimes (URL params, pagination), erreurs courantes (chaînes de redirects, canonical vers une 404).</em></p><ul><li>404 occasionnels OK, 404 massives = pénalité</li><li>Redirect 301 = permanent, transfère le PageRank</li><li>Redirect 302 = temporaire, ne transfère pas</li><li>Canonical pointe vers la version 'officielle' d'un contenu</li></ul>"},
            {'title': "Schema.org pour debutants : enrichir vos resultats Google", 'duration': 13,
             'content_html': "<p><em>Leçon enrichie à venir : ~2000 mots sur les principaux types Schema (Organization, LocalBusiness, Product, Article, FAQPage, BreadcrumbList), implémentation JSON-LD, test via Rich Results Test, impact sur les featured snippets et CTR.</em></p><ul><li>Schema = données structurées qui aident Google à comprendre votre page</li><li>JSON-LD est le format recommandé (vs Microdata, RDFa)</li><li>FAQPage Schema = featured snippets avec questions/réponses</li><li>LocalBusiness Schema = pack local + Google Maps</li><li>Test : https://search.google.com/test/rich-results</li></ul>"},
        ],
    },
    {
        ...
        'lessons': [
            {'title': "Backlinks : pourquoi et comment ils impactent votre ranking", 'duration': 20,
             'content_html': "<p><em>Leçon enrichie à venir : ~2800 mots sur le rôle des backlinks (votes de confiance), métriques d'autorité (DR Ahrefs, DA Moz), Domain Rating vs Page Authority, dofollow vs nofollow, anchor text variety, neighborhoods et toxic backlinks.</em></p><ul><li>Backlink = lien d'un autre site vers le vôtre</li><li>Vote de confiance qui transfère du PageRank</li><li>10 backlinks d'autorité &gt;&gt; 1000 backlinks low-quality</li><li>Variété d'anchor text essentielle (pas tous 'agence SEO Cotonou')</li></ul>"},
            {'title': "Strategies pour obtenir vos premiers 30 backlinks", 'duration': 22,
             'content_html': "<p><em>Leçon enrichie à venir : ~3000 mots couvrant guest posting (outreach, pitch), broken link building, linkable assets (études, calculateurs, infographies), digital PR (HARO, PressLink), partenariats locaux, citations chambre de commerce, annuaires de qualité.</em></p><ul><li>Guest post : écrire 1 article/mois sur un blog complémentaire (lien retour)</li><li>Linkable assets : créer 1 contenu 'épique' (étude 50 pages, outil gratuit)</li><li>HARO (Help A Reporter Out) : journalistes cherchent des sources, vous répondez</li><li>Citations locales : Yelp, Google Business, Pages Jaunes, annuaires sectoriels</li></ul>"},
            {'title': "SEO local : dominer Google Business Profile et le pack local", 'duration': 20,
             'content_html': "<p><em>Leçon enrichie à venir : ~2800 mots sur l'optimisation 100% complète d'une fiche GBP (catégorie, descriptions, services, attributs), photos hebdomadaires, posts GBP, gestion d'avis (collecte, réponses), citations locales NAP cohérentes, signals locaux.</em></p><ul><li>Fiche GBP 100% remplie (catégories, services, photos, descriptions)</li><li>30+ avis Google authentiques avec réponses du propriétaire</li><li>NAP (Name, Address, Phone) IDENTIQUE sur tous les annuaires</li><li>Posts GBP hebdomadaires (nouvelles, offres, événements)</li><li>Photos ajoutées chaque semaine (+12% impressions en moyenne)</li></ul>"},
            {'title': "Mesurer ses progres : Google Search Console + Analytics", 'duration': 18,
             'content_html': "<p><em>Leçon enrichie à venir : ~2500 mots sur la configuration GA4 + GSC, les KPIs SEO essentiels (impressions, clics, CTR, position moyenne), reporting mensuel template, attribuer les leads SEO, créer des dashboards Looker Studio.</em></p><ul><li>Search Console = données Google (impressions, position, clics)</li><li>GA4 = données comportement (sessions, conversion, attribution)</li><li>KPIs principaux : trafic SEO, ranking moyen top 100, conversions attribuées</li><li>Reporting mensuel : tableau standard + 3 insights clés</li><li>Dashboard Looker Studio gratuit avec template prêt à l'emploi</li></ul>"},
            {'title': "Plan d'action SEO 90 jours pour demarrer", 'duration': 15,
             'content_html': "<p><em>Leçon enrichie à venir : ~2200 mots avec planning détaillé semaine par semaine pour les 12 premières semaines (audit, recherche mots-clés, premiers articles, optimisation on-page, premiers backlinks, mesure, ajustements).</em></p><p><strong>Le plan en synthèse :</strong></p><ul><li><strong>Semaines 1-2 :</strong> Audit complet + setup tracking (GSC, GA4)</li><li><strong>Semaines 3-4 :</strong> Recherche mots-clés (50 cibles) + keyword mapping</li><li><strong>Semaines 5-8 :</strong> Optimisation on-page de 10 pages existantes + création 4 pages piliers</li><li><strong>Semaines 9-12 :</strong> Création 8 articles cluster + premières actions backlinks + mesure</li></ul><p>Resultats attendus à M+3 : +25-40% trafic organique sur les pages optimisées, premiers rankings sur 10-15 mots-clés longue traîne.</p>"},
        ],
    },
]
